...
const SavedScreen = React.memo(({navigation, route}) => {
...
// console.log('saved screen');
  ...
  const fetchSavedItems = async (tab, page = 1) => {
    ...
    // Prevent pagination when no more data
    if (isPaginating && !savedDataMap[tab]?.hasMore) {
      // console.log('No more data, skipping API call');
      return;
    }

    // console.log('Api called for tab:', tab, 'page:', page);
...
// 
    ...
    try {
      ...
      switch (tab) {
        case 'product':
          ...
          const {wishlistItems = [], pagination = {}} =
        ...
        case 'show':
          ...
          const shows = showResponse?.data?.data?.shows;
          result = {
            data: shows,
            currentPage: page,
            hasMore: false,
  ...
  */
  ...
  useEffect(() => {
    const currentTabData = savedDataMap[savedTab];
    if (currentTabData?.data.length === 0 && currentTabData?.hasMore && !loadingMap[savedTab]) {
      fetchSavedItems(savedTab, 1);
    }
  }, [savedTab]); // Only trigger when tab changes, not when data changes
  ...
  const handleUnSave = async id => {
    try {
      ...
      if (!isSaved) {
        setSavedDataMap(prev => {
          const updated = prev[savedTab].data.filter(v => v._id !== id);
          return {
            ...prev,
            [savedTab]: {
              ...prev[savedTab],
  ...
  {formatCurrency(item.MRP)}
          ...
          <View style={styles.leftContainer}>
            <View style={styles.leftButton}>
              {/* <Text style={{color: '#fff', fontSize: 9}}>Only few left!</Text> */}
             {hasDiscount && discountPercent > 0 && (
                         <LinearGradient
  ...
  const renderSavedShow = useCallback(({item}) => {
    ...
    return (
      <TouchableOpacity
        style={styles.galleryItem}
        onPress={() => {
          if (item.isLive) navigation.navigate('LiveScreen', {stream: item});
          else
        ...
        <View style={overlay.cardOverlay}>
          <View
            style={[
              styles.liveTag,
              {
                backgroundColor: !item.isLive ? '#333' : '#FF3B30',
                paddingHorizontal: !item.isLive ? 5 : 12,
              },
            ]}>
            {/* <View style={styles.liveIndicator}></View> */}
             <Text style={styles.liveText}>
              {/* {!item.isLive ? 'Upcoming' : 'LIVE'} */}
              {item?.showStatus === "live" || item?.isLive ? 'LIVE' : item?.showStatus === 'created'? 'Upcoming' : item?.showStatus === 'ended' ? 'Ended' : item?.showStatus === 'cancelled' ? 'Cancelled' :'Upcoming'}
            </Text>
          </View>
          {item.isLive ? (
    ...
    <>
      ...
      {user?.role == 'seller' || user?.role == 'user' ? (
        <SafeAreaView style={styles.container}>
          <SellerHeader navigation={navigation} message={'Saved'} />
          <FlatList
            data={getSavedData()}
            ...
            onEndReached={() => {
              const {page, hasMore} = savedDataMap[savedTab] || {};
              if (hasMore && !loadingMap[savedTab]) {
            ...
            ListEmptyComponent={
              <View style={{alignItems: 'center', justifyContent: 'center'}}>
                {loadingMap[savedTab] ? (
                  ...
                  <Text style={{color: '#777'}}>No {savedTab} Saved</Text>
                )}
              </View>
            }
            ListFooterComponent={
