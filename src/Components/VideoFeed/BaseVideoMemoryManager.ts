/**
 * 🚀 BASE VIDEO MEMORY MANAGER
 * Platform-agnostic implementation without circular dependencies
 */
...
export abstract class BaseVideoMemoryManager implements IVideoMemoryManagerInternal {
  ...
  protected async updateMemoryState(): Promise<void> {
  ...
  protected async checkForLeaks(): Promise<void> {
  ...
  protected async performMaintenanceIfNeeded(): Promise<void> {
  ...
  protected async selectVideosForCleanup(count: number): Promise<string[]> {
  ...
  protected notifyVideoRelease(videoId: string): void {
