...
const ViewReview = ({reviews,setReviews,fetchReviews}) => {
  ...
  const handleFeedback = async (reviewId, voteType) => {
    try {
        const review = reviews.find(r => r._id === reviewId);
        const currentUserVote = review?.userVote;
        ...
        // Make API call
        const response = await axiosInstance.post(`/review/${reviewId}/feedback`,
            { voteType }
        );
        ...
        fetchReviews(); // Revert to server state on error
    }
};
      ...
      <View style={styles.reviewCard}>
        ...
        <View style={styles.reviewHeader}>
          <View style={styles.userInfo}>
            {item?.userId?.profileURL?.key ?
            (<Image
              source={{
                uri: item?.userId?.profileURL?.key
                  ? `${AWS_CDN_URL}${item.userId.profileURL.key}`
                  : 'https://i.pravatar.cc/100',
              }}
              style={styles.userAvatar}
            />):(
              <FontAwesome6 name="user-circle" size={30} color='#FFC107' style={styles.userAvatar} />
            )}
